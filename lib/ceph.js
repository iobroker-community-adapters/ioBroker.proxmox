...
/**
 * Pfade (relativ zum ceph-Basis-State, ohne Namespace), die beim Traversieren
 * komplett übersprungen werden sollen.
 * Format: "ceph.<unterordner>.<schlüssel>"  – der Namespace-Präfix wird zur Laufzeit vorangestellt.
 * Einfach neue Einträge hinzufügen um weitere Pfade auszuschließen.
 */
const CEPH_EXCLUDED_PATHS = [
    'mgrmap.available_modules',
];
...
async function traverseCeph(ctx, baseId, data, depth, path, createObjects, onState, visitedPaths = new Set()) {
    ...
    for (const [key, value] of Object.entries(data)) {
        ...
        // Pfad-Ausschluss: stateId endet mit einem der konfigurierten Suffixe (ceph.<pfad>)
        const relativeStateId = stateId.replace(/^[^.]+\.[^.]+\./, ''); // entfernt "namespace.proxmox."
        const cephRelative = stateId.replace(/^.*?\.ceph\./, '');        // entfernt alles bis inkl. ".ceph."
        if (CEPH_EXCLUDED_PATHS.some(ex => cephRelative === ex || cephRelative.startsWith(`${ex}.`))) {
            ctx.log.debug(`traverseCeph: Pfad ausgeschlossen – ${stateId}`);
            continue;
        }
        ...
        if (Array.isArray(value)) {
            for (let i = 0; i < value.length; i++) {
                ...
                if (typeof value[i] === 'object' && value[i] !== null) {
                    if (createObjects) {
                        await ctx.setObjectNotExistsAsync(arrId, {
                            type: 'folder',
                            common: { name: `${key}[${i}]` },
                            native: {},
                        });
...
async function setCeph() {
    ...
    try {
        ...
        await traverseCeph(
            this, cephid, cephInformation.data, 1, cephid, false,
            async (id, val) => this.setStateChangedAsync(id, val, true)
        );
    } catch (err) {
