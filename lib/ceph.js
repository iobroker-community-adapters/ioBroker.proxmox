...
/**
 * Traversiert cephInformation.data bis Tiefe 3.
 ...
 */
async function traverseCeph(ctx, baseId, data, depth, path, createObjects, onState) {
    if (depth > 3) return;
    ...
    for (const [key, value] of Object.entries(data)) {
        ...
        if (typeof value === 'object') {
            if (depth < 3) {
                ...
                await traverseCeph(ctx, baseId, value, depth + 1, stateId, createObjects, onState);
            }
        } else {
            if (createObjects) {
