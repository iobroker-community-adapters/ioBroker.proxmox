...
async function traverseCeph(ctx, baseId, data, depth, path, createObjects, onState, visitedPaths = new Set()) {
    ...
    for (const [key, value] of Object.entries(data)) {
        ...
        if (Array.isArray(value)) {
            // Array-Container-Ordner immer sicherstellen (Array kann dynamisch wachsen)
            ...
            for (let i = 0; i < value.length; i++) {
                ...
                if (typeof value[i] === 'object' && value[i] !== null) {
                    // Element-Ordner immer sicherstellen
                    ...
                    // Primitive States im Array immer sicherstellen
                    await ctx.extendObjectAsync(arrId, {
                        type: 'state',
