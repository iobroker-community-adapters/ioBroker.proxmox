...
class ProxmoxUtils {
    ...
    async _getData(url, method, data, retry) {
        return new Promise((resolve, reject) => {
            if (this.stopped) {
                reject('STOPPED');
            }
            ...
            axios({
                ...
                .catch((error) => {
                    this.adapter.log.error(`Error received ${error.response.status} response from ${url} with content: ${JSON.stringify(error.response.data)}`);
