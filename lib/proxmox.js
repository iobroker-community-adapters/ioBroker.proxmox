...
proxmox.prototype._getTicket = function (callback) {
    ...
    request.post({
        url: `${URL}/access/ticket?username=${encodeURIComponent(this.name)}@${encodeURIComponent(this.server)}&password=${encodeURIComponent(this.password)}`,
        'strictSSL': false
    }, (err, res, body) => {
        if (err) {
