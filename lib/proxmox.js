...
class ProxmoxUtils {
    ...
    async _getData(url, method, data, retry, additional) {
        return new Promise((resolve, reject) => {
            ...
            const pathU = url || '';
            this.adapter.log.debug(`[_getData] starting request to --> ${this.URL}${pathU} ...`);
            ...
            axios({
                ...
                .then((response) => {
                    this.adapter.log.debug(`[_getData] received ${response.status} response from ${pathU} with content: ${JSON.stringify(response.data)}`);
                    this.communicationErrorCounter = 0;
    ...
    async _getTicket() {
        return new Promise((resolve, reject) => {
            ...
            axios({
                ...
                .then((response) => {
                    this.adapter.log.debug(`[_getTicket] received ${response.status} response from proxmox with content: ${JSON.stringify(response.data)}`);

                    if (response.status === 200) {
                        this.adapter.log.debug(`[_geTicket] dataticket: ${JSON.stringify(response.data)}`);
