...
class ProxmoxUtils {
    constructor(adapter) {
        ...
        this.ipList = adapter.config.ip
            ...
            .map((el) => el.trim())
            .filter((el) => el.length); */
        this.name = adapter.config.server[0].name;
        this.server = adapter.config.server[0].server || 'pam';
        this.password = adapter.config.server[0].pwd;
        this.port = adapter.config.server[0].port;
        this.ip = adapter.config.server[0].ip;
 ...
 */
        ...
        if (this.adapter.config.server){
            if (this.currentIpId < this.adapter.config.server.length) {
                this.name = this.adapter.config.server[this.currentIpId].name;
                this.server = this.adapter.config.server[this.currentIpId].server;
                this.password = this.adapter.config.server[this.currentIpId].pwd;
                this.port = this.adapter.config.server[this.currentIpId].port;
                this.ip = this.adapter.config.server[this.currentIpId].ip;
            }
        }
